...
public class CommandListener implements Listener {
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent e) {
        ...
        if (config.getBoolean("ignore-unregistered-commands") &&
        ...
        int messageStart;
        if (config.getBoolean("spy-teammsg") &&
            ...
            messageStart = 1;
        } else if (containsCommand(config.getStringList("social-commands"), command)) {
            if (args.length < 3)
                return;
            ...
            messageStart = 2;
        } else if (containsCommand(config.getStringList("reply-commands"), command)) {
            if (args.length < 2)
                return;
            ...
            if (containsCommand(config.getStringList("ignored-commands"), command) ^
                    config.getBoolean("invert-ignored-commands"))
                return;
            boolean blacklisted = config.getBoolean("blacklist-is-whitelist") ^
                    containsCommand(config.getStringList("command-blacklist"), command);
        ...
        if (config.getBoolean("command-spy-on-social-commands"))
    ...
    private boolean containsCommand(List<String> commands, String command) {
        ...
        boolean ignoreAliases = config.getBoolean("ignore-aliases");
